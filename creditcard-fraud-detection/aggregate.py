...
def train(federated_averaging_process, num_rounds, num_clients_per_round):
  ...
  for round_num in range(num_rounds):
      ...
      print('round {:2d}, metrics={}, broadcasted_bits={}, aggregated_bits={}'.format(round_num, metrics,
                                                                                      format_size(broadcasted_bits),
                                                                                      format_size(aggregated_bits)))
      accuracy.append(metrics.get("train").get("sparse_categorical_accuracy"))
      loss.append(metrics.get("train").get("loss"))
      broadcasted.append(float(format_size(broadcasted_bits).split("G")[0]))
      aggregated.append(float(format_size(broadcasted_bits).split("G")[0]))
  ...
  return accuracy, loss, broadcasted, aggregated
...
def show(a, b, c,title, y_label):
    x = list(range(1, epoch+1))

    plt.plot(x, a, label='FedAvg')
    plt.plot(x, b, label='local')
    plt.plot(x, c, label='period')
    # 设置X刻度
...
if __name__ == "__main__":
    ...
    federated_averaging_with_compression1 = tff.learning.build_federated_averaging_process(
        tff_model_fn,
        client_optimizer_fn=lambda: tf.keras.optimizers.SGD(learning_rate=0.02),
        server_optimizer_fn=lambda: tf.keras.optimizers.SGD(learning_rate=1.0))
    print("没有压缩结果： ")
    accuracy, loss, broadcasted, aggregated = train(federated_averaging_with_compression1, epoch, 100)
    ...
    print("压缩结果： ")
    accuracy1, loss1, broadcasted1, aggregated1 = train(federated_averaging_with_compression, epoch, 100)

    federated_averaging_with_compression2 = tff.learning.build_federated_averaging_process(
        tff_model_fn,
        client_optimizer_fn=lambda: tf.keras.optimizers.SGD(learning_rate=0.02),
        server_optimizer_fn=lambda: tf.keras.optimizers.SGD(learning_rate=1.0),
        aggregation_process=encoded_mean_process)
    print("压缩结果： ")
    accuracy2, loss2, broadcasted2, aggregated2 = train(federated_averaging_with_compression2, epoch, 100)

    show(accuracy, accuracy1,accuracy2, "accuracy", "accuracy")
    show(loss, loss1, loss2,"loss", "loss")
    show(broadcasted, broadcasted1,broadcasted2, "broadcasted", "broadcasted")
    show(aggregated, aggregated1, aggregated2,"aggregated", "aggregated")
